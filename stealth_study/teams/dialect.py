...
- RemoteDialect: one wire protocol (the `/v1/board` HTTP API) to a board served
  elsewhere — the running Stealth Study sidecar on this machine, a teammate's machine,
...
Every front door — the `team-board` MCP server, the `ocw` CLI, remote Stealth Study
...
class RemoteDialect:
    """The `/v1/board` HTTP client. `base_url` is an Stealth Study sidecar or a hosted
...
def local_dialect(
    ...
    backing for the CLI and MCP server when no Stealth Study server is running."""
