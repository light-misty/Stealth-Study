"""Launch the server with uvicorn. Used by the desktop GUI sidecar and `hiu-workspace-server`."""
...
def _exit_when_orphaned() -> None:
    ...
    crash) that skips the shell's graceful child-kill. Standalone `hiu-workspace-server` runs are
    unaffected.
...
def main(argv=None) -> None:
    ...
    parser = argparse.ArgumentParser(prog="hiu-workspace-server")
