...
def _browser_page(
    ...
    return (
        ...
        f"<title>{_html.escape(title)} — HIU WorkSpace</title><style>"
        ...
        "</style></head><body>"
        '<div class="card"><div class="mark"><i></i>HIU WorkSpace</div>'
        f"{icon}<h1>{_html.escape(title)}</h1><p>{_html.escape(detail)}</p>{err}</div>"
        '<div class="foot">Served locally by HIU WorkSpace on your Mac</div>'
        "</body></html>"
    )
...
_CONNECT_FAILED_DETAIL = (
    ...
    "Close this tab and try again from HIU WorkSpace."
)
...
def create_app(manager: SessionManager) -> FastAPI:
    ...
    def _request_authenticated(request: Request) -> bool:
        provided = request.headers.get("x-hiu-workspace-token", "")
        return bool(
            api_token
            and provided
    ...
    async def require_sidecar_token(request: Request, call_next):
        ...
        return JSONResponse(
            {"error": "missing or invalid HIU WorkSpace sidecar token"},
            status_code=401,
        )
    ...
    async def mcp_oauth_callback(
        ...
        return HTMLResponse(
            _browser_page(
                "Connected",
                "Sign-in complete. You can close this tab and return to HIU WorkSpace.",
                ok=True,
            )
        )
    ...
    # -- HIU WorkSpace Cloud: sign-in + managed one-click connect ---------------
    ...
    async def cloud_auth_callback(code: str = "", state: str = "", error: str = ""):
        ...
        signin_failed_detail = (
            "Close this tab and try signing in again from HIU WorkSpace."
        )
        if error:
            return HTMLResponse(
                _browser_page(
        ...
        return HTMLResponse(
            _browser_page(
                "Signed in",
                "You're signed in to HIU WorkSpace Cloud. "
    ...
    async def ws_session(ws: WebSocket, session_id: str) -> None:
        ...
        if not _origin_allowed(ws.headers.get("origin")):
            await ws.close(code=1008)
            return
        await ws.accept(subprotocol="hiu-workspace" if api_token else None)
