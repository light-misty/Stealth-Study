...
def _parse_or_coerce(target: str) -> tuple[str, str, Optional[str]]:
    """parse_target, but a BARE channel name ('all-hiu-workspace', '#general') coerces to
...
def _resolve_slack_channel(
    ...
    """'#all-hiu-workspace' (a NAME the user said) → the team-qualified chat_id, via the
    ...
    if not c.get("is_member"):
        return None, (
            f"found #{query}, but the bot isn't a member — invite @HIU WorkSpace to #{query} "
            "in Slack, then retry"
        )
    return chat_id, None
