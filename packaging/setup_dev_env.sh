...
# stealthstudy-server directly, and the Tauri desktop shell falls back to it when
...
python3 -m venv "$VENV"
# The stealth_study package (server, engine, connectors) + inbound-messaging extras.
...
"$VENV/bin/python" -c 'import aisuite, stealth_study' # fail loudly if the wiring broke
echo "Ready: $VENV"
echo "  server: $VENV/bin/stealthstudy-server --cwd /path/to/your/project --port 8765"
