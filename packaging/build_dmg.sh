...
echo "==> [1/5] PyInstaller: bundling stealthstudy-server ($TRIPLE)"
"$PLATFORM/.venv/bin/pyinstaller" --noconfirm --clean \
  --distpath "$HERE/dist" --workpath "$HERE/build" "$HERE/stealthstudy-server.spec"
...
rm -rf "$GUI/src-tauri/binaries/sidecar" "$GUI/src-tauri/binaries/stealthstudy-server-$TRIPLE"
...
cp -RL "$HERE/dist/stealthstudy-server" "$GUI/src-tauri/binaries/sidecar"
...
chmod +x "$GUI/src-tauri/binaries/sidecar/stealthstudy-server"
...
if [ -n "${APPLE_SIGNING_IDENTITY:-}" ]; then
  ...
  find "$SIDECAR" -type f ! -name "stealthstudy-server" \
  ...
  codesign --force --sign "$APPLE_SIGNING_IDENTITY" --timestamp --options runtime \
    --entitlements "$GUI/src-tauri/entitlements.plist" "$SIDECAR/stealthstudy-server"
fi
