...
#   3. `tauri build --bundles app` → HIUWorkSpace.app (resources are copied in).
...
echo "==> [1/5] PyInstaller: bundling hiu-workspace-server ($TRIPLE)"
"$PLATFORM/.venv/bin/pyinstaller" --noconfirm --clean \
  --distpath "$HERE/dist" --workpath "$HERE/build" "$HERE/hiu-workspace-server.spec"
...
rm -rf "$GUI/src-tauri/binaries/sidecar" "$GUI/src-tauri/binaries/hiu-workspace-server-$TRIPLE"
...
cp -RL "$HERE/dist/hiu-workspace-server" "$GUI/src-tauri/binaries/sidecar"
...
chmod +x "$GUI/src-tauri/binaries/sidecar/hiu-workspace-server"
...
if [ -n "${APPLE_SIGNING_IDENTITY:-}" ]; then
  ...
  find "$SIDECAR" -type f ! -name "hiu-workspace-server" \
  ...
  codesign --force --sign "$APPLE_SIGNING_IDENTITY" --timestamp --options runtime \
    --entitlements "$GUI/src-tauri/entitlements.plist" "$SIDECAR/hiu-workspace-server"
fi
