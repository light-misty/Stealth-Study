...
### 0.0 v1.1 变更摘要（相对 v1.0 Draft）

> v1.1 的主线：**把 §13 架构师评审中"与代码事实不符"的部分，全部吸收进正文。** 共修订 12 类 A/B 项 + 3 项产品决策。
...
- **点击后发生什么**：点"提交批改" → 调 `coworker/campus/grading.py`（基于 reviewer + `cet-essay-grading` / `cet-translation-grading` 技能包）→ 返回结构化 JSON → 渲染 + 落库 → 错误项自动入错题本（按错误类型归因）。
- **数据存哪**：`campus.db` → `attempt`（含原文、批改 JSON、分数）+ `mistake_book`。
- **复用**：**Reviewer（核心）**、Skills（评分标准固化）、Memory（记住用户常犯错误，下次批改重点关注）。
- **用户故事**：作为林晓，我希望我写的作文有人按四六级的标准改，并且告诉我老毛病是什么，以便考前改掉。
- **验收标准**：
  1. 同一篇作文提交两次，分项得分**波动不超过 1 档**（证明评分标准被固化，不是随机的）；
...
- **复用**：Skills（`mock-exam-proctor`）、Tools（`todo` 生成考务清单）、Reviewer（主观题判分）。
...
- **点击后发生什么**：填表 → `kaoyan-planner` 人设 + `plan` 工具 → 生成**四轨并行**的 `study_plan`（每轨独立阶段），并生成 `plan_task`（周级 + 日级）→ 写入 Teams 看板（待学/在学/待复习/已掌握）。
- **数据存哪**：`campus.db` → `study_plan`（含 `track` 字段区分轨）+ `plan_task`；看板走既有 Teams。
- **复用**：Persona（`kaoyan-planner`）、Tools（plan/todo）、Teams board、Automation（周报）。
- **用户故事**：作为陈默，我希望输入目标院校和科目后得到一份分科目的阶段计划，以便不必自己搭 Notion。
- **验收标准**：
  1. 选择"考数二 + 英二"与"不考数学 + 两门专业课"生成的计划结构**明显不同**（证明异构）；
  2. 计划覆盖到考试日期，周级任务数 = 周数，每轨都有任务（不会某轨为空）；
  3. 任务同步到看板，拖动看板卡片状态会写回 `plan_task.status`；
...
| **专业课** | 资料列表 + 问答区 + 真题/自命题录入 | 基于导入资料的检索问答（Compaction 支撑长文档）+ 自命题真题录入与解析 | `source_doc` + `question_bank_item` |
...
- **复用**：Persona（`kaoyan-subject-tutor`）、Compaction（长资料）、Skills、Reviewer（批改）、PDF（资料）。
...
- **页面有什么**：
  ...
  - "基于本章生成 5 道题"按钮。
- **点击后发生什么**：提问 → 后端检索切片 → 调模型（长上下文走 Compaction）→ 返回带引用答案；点"生成题目" → 生成 `question_bank_item` 并进入刷题。
- **数据存哪**：`campus.db` → `source_doc`（切片元信息）+ 问答记录进 `attempt` / journal。
- **复用**：**PDF 支持 + attachments（核心）**、**Compaction（核心）**、Tools（files）、Persona。
- **用户故事**：作为陈默，我希望直接问我那份 200 页的专业课扫描件，并且答案告诉我在第几页，以便我能回去核对。
- **验收标准**：
  1. 上传 ≥50 页 PDF，连续追问 10 轮，不出现前后矛盾或上下文丢失（Compaction 生效）；
  2. 每个回答至少 1 条可点击引用；
  3. 生成题目后可直接作答并判分；
  4. 全程离线可用（Ollama 场景）。
...
- **点击后发生什么**：开始刷题 → 逐题作答 → 客观题自动判分、主观题调 Reviewer 批改 → 提交后写入 `attempt` → 错题进入 CERT3。
- **数据存哪**：`campus.db` → `question_bank_item` / `question_option`（可选） / `attempt`。
- **复用**：**Reviewer（主观题）**、Skills、PDF（资料出题）、Tools（files 导入）。
...
- **页面有什么**：
  ...
  - **归因五分类（固定枚举）**：`概念不清` / `审题失误` / `计算或操作失误` / `超纲或不熟` / `时间不够`。AI 自动打标 + 用户可改。
...
- **复用**：**Memory（长期记忆薄弱点）**、**Automation（复习推送）**、Skills（`mistake-attribution`）、Reviewer。
- **用户故事**：作为苏晴，我希望知道我是"不会"还是"看错题"，以便对症下药。
- **验收标准**：
  1. 每条错题必须有归因标签（AI 自动打标准确率人工抽检 ≥70%，用户可修改）；
  2. 归因分布图与列表筛选联动；
  3. 同一知识点错题 ≥2 条时，列表顶部提示"该知识点你有 N 道错题"；
  4. 加入复习队列后，次日收到推送且推送内容包含该题；
...
- **页面有什么**：
  ...
  - 支持：① 手动填写；② 从证书模板预填（模板只含"通常月份"，如"教资笔试 3 月/9 月，报名通常在考前 2 个月"，**明确标注为参考值，需用户核对**）；③ 一键创建自动化提醒（D-30 / D-7 / D-1）。
- **点击后发生什么**：点"创建提醒" → 用既有 Automation 创建 3 个定时任务 → 到点推送（走既有推送通道）。
- **数据存哪**：`campus.db` → `cert_deadline`；提醒任务走既有 `coworker/automation/`。
- **复用**：**Automation（核心）**、Tools（search 可选联网核对时间）。
- **用户故事**：作为苏晴，我希望不再因为刷小红书才知道报名结束了。
- **验收标准**：
  1. 手动填写日期后时间轴与倒计时正确；
  2. 点击创建提醒后，自动化列表中出现 3 条任务且 cron 正确；
  3. 提醒触发后，用户收到通知且内容含节点名与剩余天数；
...
| **最依赖的已有能力** | Reviewer（批改）、Skills（评分标准）、Memory（常犯错误） | Automation（周报）、Teams（看板）、Compaction（长资料）、PDF | Skills（知识点抽取）、Reviewer（rubric）、Automation（节点提醒） |
...
| G-05 | G4 减法 | `features.voice = false` 配置与全局短路 | P0 | 配置系统 | 低 | 无语音入口、无麦克风请求 | 【新增】+【隐藏/关闭】 |
| G-06 | G4 减法 | `features.login = false` 配置与登录 UI 短路 | P0 | 配置系统 | 低 | 无登录/注册/账号区 | 【新增】+【隐藏/关闭】 |
| G-07 | G4 减法 | 连接器 OAuth 降级为可选且不阻断主流程 | P0 | 既有连接器 | 低 | 不授权时学习功能 100% 可用 | 【隐藏/关闭】 |
| G-08 | G5 i18n | `campus.*` 命名空间 zh/en 追加 | P0 | i18next | 中 | 双语无硬编码残留 | 【新增】 |
| G-09 | G6 设置 | 备考偏好设置 tab（时长/推送时间/模型/间隔强度） | P1 | SettingsView tab 机制 | 中 | 设置持久化且生效 | 【新增】 |
| G-10 | G7 资料库 | 资料导入（PDF/MD/TXT）与切片入索引 | P0 | PDF 支持 + attachments + files | 中 | 20 页 PDF 就绪且可问答 | 【新增】 |
| G-11 | G7 资料库 | 资料列表管理与删除 | P1 | files | 低 | 删除后文件与记录清理 | 【新增】 |
| G-12 | 人设 | 6 个学习人设目录（含 manifest + skills） | P0 | Persona 系统 | 中 | 人设列表出现且可调用 | 【新增】 |
| G-13 | 技能 | 7 个学习技能包（批改/精听/归因/周报/知识点树/模考/讲解） | P0 | Skills 系统 | 高 | 技能可被 Persona 加载并执行 | 【新增】 |
| G-14 | 后端 | `coworker/campus/` 包骨架 + `campus.db` 建表 | P0 | — | 中 | 表结构按 §6 建立，既有库不受影响 | 【新增】 |
| G-15 | 后端 | `coworker/campus/grading.py`（基于 reviewer 的结构化批改） | P0 | Reviewer | 中 | 批改返回结构化 JSON，两次一致 | 【新增】 |
| G-16 | 共享 | 统一错题本（三台汇入，按 track/subject 区分） | P0 | Memory + Reviewer | 中 | 三台错题均可见且筛选正确 | 【新增】 |
| G-17 | 共享 | 间隔复习调度器（简化 SM-2）+ `review_queue` | P0 | Automation | 中 | 队列按间隔生成，推送可达 | 【新增】 |
| G-18 | 共享 | 4 个自动化模板（每日复习/周报/冲刺/节点提醒） | P0 | Automation | 中 | 模板可一键创建，cron 正确 | 【新增】 |
...
| CET-09 | CET4 | 作文批改（分项得分 + 错误清单 + 升格示范） | P0 | **Reviewer** + Skills | 高 | 两次批改分差 ≤1 档 | 【新增】 |
| CET-10 | CET4 | 翻译逐句批改 | P1 | **Reviewer** + Skills | 中 | 逐句给出修改建议 | 【新增】 |
...
| CET-14 | CET5 | 模考估分与分项得分率 | P1 | Reviewer | 中 | 估分非空，错题入本 | 【新增】 |
...
| KY-03 | KY1 | 计划同步到 Teams 看板（待学/在学/待复习/已掌握） | P1 | **Teams board** | 中 | 拖卡写回任务状态 | 【新增】 |
| KY-04 | KY1 | 修改考试日期后一键重排 | P1 | plan 工具 | 中 | 已完成任务不丢失 | 【新增】 |
| KY-05 | KY2 | 政治轨（时政卡 + 章节 + 分析题"点—析—结"批改） | P1 | Persona + Reviewer | 中 | 结构化得分点 | 【新增】 |
| KY-06 | KY2 | 英语轨（长难句拆解 + 阅读精析 + 写作批改） | P1 | Persona + Reviewer | 中 | 长难句三段标注 | 【新增】 |
| KY-07 | KY2 | 数学轨（考纲章节树 + 分步解题 + 错题按知识点聚合） | P0 | Persona + Reviewer | 中 | 同知识点错题聚合提示 | 【新增】 |
| KY-08 | KY2 | 专业课轨（真题录入 + 解析） | P1 | Persona | 中 | 录入后可作答判分 | 【新增】 |
| KY-09 | KY3 | 专业课资料问答（带页码/段落引用） | P0 | **PDF + Compaction** | 高 | 10 轮追问不丢上下文 | 【新增】 |
...
| CERT-07 | CERT2 | 主观题批改（简答/论述/材料分析/教案/实操步骤） | P0 | **Reviewer** | 高 | 结构化得分点 | 【新增】 |
| CERT-08 | CERT3 | 错题本（五分类归因 + 自动打标 + 人工可改） | P0 | Reviewer + Skills | 高 | 抽检准确率 ≥70% | 【新增】 |
| CERT-09 | CERT3 | 归因分布图与"头号杀手" | P1 | — | 低 | 与筛选联动 | 【新增】 |
...
| CERT-14 | CERT5 | 自定义评分标准（rubric）与按 rubric 批改 | P1 | Reviewer | 中 | 结果按 rubric 组织 | 【新增】 |
...
| 需求ID | 所属模块 | 需求描述 | 优先级 | 复用已有能力 | 复杂度 | 验收标准 | 标记 |
|---|---|---|---|---|---|---|---|
| INF-01 | 数据 | `campus.db` 建表（见 §6），不动 `coworker.db` | P0 | — | 中 | 建表成功，既有库无变化 | 【新增】 |
| INF-02 | 数据 | 学习数据导出（Markdown / JSON / CSV） | P1 | files | 低 | 导出完整可再导入 | 【新增】 |
| INF-03 | 数据 | 学习数据导入（备份恢复） | P2 | files | 中 | 导入后数据一致 | 【新增】 |
| INF-04 | 前端 | `components/campus/` 组件目录与路由接入 | P0 | App.tsx 三元链 | 低 | 路由可切换 | 【新增】 |
| INF-05 | 前端 | `campus/api.ts` + `campus/types.ts`（不动既有 api.ts/types.ts） | P0 | — | 中 | 类型完整 | 【新增】 |
| INF-06 | 测试 | 新组件 Vitest 测试（对齐既有测试风格） | P1 | Vitest | 中 | 覆盖率 ≥ 既有平均 | 【新增】 |
| INF-07 | 测试 | 语音/登录关闭路径的回归测试 | P1 | Vitest | 低 | flag 关闭时无相关渲染 | 【新增】 |
| INF-08 | 配置 | `campus` 配置 section 与 `features` 开关读取器 | P0 | 配置系统 | 低 | 配置项生效 | 【新增】 |

**需求统计**：共 **66 条**。P0 = 34 条，P1 = 25 条，P2 = 7 条。
...
sequenceDiagram
    ...
    loop 逐题作答（可中断）
        ...
        API->>BE: 判分（客观题本地判，主观题走 Reviewer）
...
| 异常 | 表现 | 处理 |
|---|---|---|
| 未配置任何模型 | 定级测评点不动 | 引导到设置页配置 Key 或 Ollama，并展示能力矩阵（哪些功能可用） |
...
flowchart TD
    ...
    B -->|"客观题"| C["本地判分：对照标准答案"]
    B -->|"主观题"| D["Reviewer + 技能包按 rubric 判分"]
    C --> E["写入 attempt 表<br/>is_correct=false"]
    D --> E
    E --> F["触发归因：<br/>mistake-attribution 技能包"]
    F --> G["AI 归因五分类：<br/>概念不清 / 审题失误 / 计算或操作失误 / 超纲或不熟 / 时间不够"]
    G --> H{"置信度 >= 阈值"}
    H -->|"是"| I["自动写入 mistake_book.attribution"]
    H -->|"否"| J["标记为『待确认』<br/>在错题本中提示用户手动选择"]
    I --> K["关联知识点：knowledge_point_id<br/>同知识点错题数 +1"]
    J --> K
    K --> L["写入 Memory：<br/>『用户 XX 知识点薄弱』长期记忆"]
    L --> M{"用户操作"}
    ...
    W --> N
    X --> N
    P --> Y["知识点掌握度提升<br/>CERT1 覆盖率 + / KY2 数轨提示消失"]
```
...
flowchart LR
    ...
    C --> D["同步到 Teams 看板：待学/在学/待复习/已掌握"]
    D --> E["每日执行：用户拖卡或勾选任务"]
...
| **任务看板** | **【复用】** 既有 Teams board / journal | 不自建看板 |
| **调度** | **【复用】** 既有 Automation 表 | 不自建调度 |

> 架构师需确认 `state_dir()` 的实际解析路径，以决定 `campus.db` 的最终落点。产品侧只要求：**与既有库同目录、独立文件、可单独删除而不影响主程序**。
...
// plan_task
{
  ...
  "board_card_id": "string | null",   // 关联既有 Teams 看板卡片
...
| 表 | 关键字段 |
|---|---|
| `cert_deadline` | `profile_id`, `node_type`（报名开始/截止/缴费截止/准考证/考试/成绩）, `date`, `is_reference`（模板预填=true，需展示"以官方为准"）, `automation_ids`（JSON 数组，关联创建的自动化任务） |
| `source_doc` | `profile_id`, `title`, `file_path`（相对 `campus/library/`）, `file_type`, `page_count`, `parse_status`（pending/ready/failed）, `chunk_count`, `imported_at` |
...
| 资料问答（10 轮） | 单轮 ≤ 10 秒（云端） | Compaction 生效，不丢上下文 |
| 错题本列表（1000 条） | 首屏 ≤ 500ms | 分页/虚拟滚动 |

**本地模型（Ollama）场景的专门要求：**
- 所有 AI 调用必须有**超时 + 可中断**；
- 提供**模型能力自检**：用既有 `providers/capabilities.py` 与 `matrix.py` 判断当前模型是否支持结构化输出；不支持时给出降级（如改为要求模型输出特定标记后解析）或提示切换模型；
- 提供**任务 × 模型推荐矩阵**（批改用强模型、出题用便宜模型），写入人设 manifest 的 `recommended_models`。
...
> **MVP 标准：一个没考过的二本大二学生，装上后不配置任何账号（可配一个模型 Key），能在 30 分钟内完成四六级定级测评、看到自己的分数差距、拿到一份到考试日的每日计划，并且第二天晚上 8 点收到复习推送。**

**P0 需求清单（34 条）：** 见 §4.8 中标记为 P0 的条目。按模块聚合：

- **全局**：G-02/03（导航 + 档案）、G-05/06/07（语音关闭 + 零登录）、G-08（i18n）、G-10（资料导入）、G-12/13（人设 + 技能）、G-14/15（后端骨架 + 批改）、G-16/17/18（错题本 + 复习调度 + 自动化模板）、INF-01/04/05/08
- **CET**：CET-01/02/03（定级 + 拆解 + 计划）、CET-04/05（高频词 + 复习）、CET-09（作文批改）
- **KY**：KY-01/02（规划表单 + 四轨计划）、KY-07（数学轨）、KY-09（资料问答）、KY-11/12（进度 + 周报）
- **CERT**：CERT-01/02/03（知识点树）、CERT-04/05（题库 + 刷题）、CERT-07（主观题批改）、CERT-08（错题归因）、CERT-11/13（节点 + 提醒）
...
| T2 | **上下文长度**：专业课 200 页 PDF、10 轮追问 | 中 | 中 | 依赖既有 Compaction；需在 V0.1 用真实大文档验证一次，若失效则改为"按章节切片 + 检索"，不依赖长上下文 |
| T3 | **PDF 解析质量**：扫描件（图片型 PDF）无文字层 | 高 —— 考研专业课大量是扫描件 | 中 | ① V0.1 只支持有文字层的 PDF，扫描件明确提示"暂不支持（需 OCR）"；② OCR 方案（本地 PaddleOCR/Tesseract）列为 V0.3+ 候选，需评估体积与依赖；③ 提供替代路径：用户手动录入/粘贴 |
| T4 | **结构化输出不稳定**：模型不按 JSON schema 返回 | 中 | 高 | ① 提示词固化在技能包中；② 后端做 JSON 修复重试（1 次）；③ 关键字段缺失时降级为纯文本 + 标注；④ 记录失败率作为模型能力评估输入 |
| T5 | **批改一致性**：同一篇作文两次批改分差大 | 中 —— 直接影响用户信任 | 中 | ① 评分标准写进技能包（四档/分项 rubric）；② 温度设低；③ 输出必须含得分点级证据；④ 验收标准明确"波动 ≤1 档" |
...
| # | 风险 | 影响 | 概率 | 缓解措施 |
|---|---|---|---|---|
| P1 | **配置模型 Key 的门槛劝退用户**（假设 A1） | 高 | 中 | ① 首次启动的分步引导（推荐 Ollama 一条命令）；② 明确"无模型也能用"的功能清单；③ 模型能力自检卡片 |
...
| 依赖 | 说明 | 风险 |
|---|---|---|
| `coworker/automation/` 的 cron 调度与推送通道 | 每日复习与节点提醒完全依赖 | 需验证桌面端通知在目标 OS 上可用 |
...
| **Q2** | 语音与登录的处置是否接受"Flag 屏蔽 + 代码保留"？<br/>还是必须物理删除文件？ | A Flag 屏蔽保留代码（推荐）/ B 物理删除 | **A**。理由见 §3.3.2：物理删除需大规模改动既有文件与测试，恰恰最违反"不大幅修改"；Flag 屏蔽在用户可感知层面完全等价，且可逆 | **高（阻塞 G-05/06/07）** |
| **Q3** | 是否接受"Core 学习功能在无模型时不可用"？<br/>（即必须配 Key 或装 Ollama 才能批改/讲解/规划） | A 接受（推荐）/ B 必须内置免费模型额度 | **A**。B 会引入服务器与账号，破坏本地优先与零登录 | **高（决定架构）** |
...
| **Q9** | ~~是否接受 MVP 只开放四六级一个台（V0.1），考研与证书放到 V0.2？~~ **【已决策】** | A 接受 / B 三个台一起上 | **B（用户已拍板，2026-09-08）**。V0.1 三个备考台同时上线，内部按 CET→KY→CERT 优先级分层推进，不分期；开发周期相应上调（见 §8.2 V0.1，6–8 周） | 已解决 |
