...
## 8. 用户拍板后的处置（2026-09-08 追加）

| 原条目/风险 | 用户拍板 | 处置 |
|---|---|---|
| R1 STT 多语种/中文模型（盘点条目 #12，C 档） | **本项目不做任何语音相关功能**，并关闭、不显示原有语音识别入口 | B-04 由「换中文模型」改写为「语音功能边界：不做并隐藏」（Composer 语音按钮隐藏、STT 组件不随主流程启用）；四六级 C1-05 听力口语、考研 C2-05 复试口语删除；语音相关风险解除 |
| R2 人设分发（盘点条目 #9） | 人设改为「AI 老师」向（用户是学生，AI 以老师角色讲解）；**弃用 hiu 前缀**（旧方案已推翻） | 人设命名改为 `xueban-cet` / `xueban-kaoyan` / `xueban-cert`（与产品名「学伴 AI / StudyBuddy」一致）；落盘目录由 `.hiu/notes/` 改为 `.xueban/notes/`；分发机制（ships:true 还是改构建脚本）仍为唯一阻断级待确认 |
| R3 本地 Python sidecar 启动与首启引导 | 打包生产版本时应完整打包、安装即用；现有项目**已实现安装即用**与首次打开配置 API Key，沿用即可 | R3 从阻断级清单移除，无首启引导需求；R4（BYOK 门槛）降级为低 |
