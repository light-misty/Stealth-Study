...
- 文档编号：PRD-StudyBuddy-v1.2-20260908
- 版本：v1.2（删除教师端「仅描述不实现」内容；风险章节上移为第 10 章）
- 日期：2026-09-08
- 产品经理：许清楚（Xu）
- 依赖输入：架构师能力盘点文档 `1-能力盘点-学伴AI.md`
- 文档语言：简体中文
...
仓库内部代号保留 `HIU-WorkSpace` / `coworker`；本文档对外产品名重新设计为「学伴 AI（StudyBuddy）」，人设前缀使用 `xueban-*`（弃用 hiu 前缀）。本产品仅面向学生单机场景，不含教师端与管理端。
...
> 原 R1（STT 模型选型）与 R3（sidecar 启动与首启引导）已按用户拍板解除，见 10.1。
...
| v1.2 | 2026-09-08 | 许清楚（产品经理） | 删除教师端「仅描述不实现」内容（原 2.3 节与原第 10 章双端边界章节）；风险与待确认清单上移为第 10 章 |
