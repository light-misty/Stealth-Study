...
- **决策**：① `ss/campus/models.py` 维护 `{任务 → [推荐模型, 最低可用模型]}` 静态清单，UI 明示建议；② 结构化输出以 `provider.complete(**{"response_format": {...}})` 试错（Ollama 走 `OpenAIProvider`，`openai_provider.py:189-193` 透传；`:92-120` 对未列举参数 re-raise，故 try/except 后去参重试）。
