...
flowchart TB
    ...
    subgraph KERNEL["既有运行时内核 coworker/  【不改动】"]
        PROV["providers/<br/>模型路由（含 Ollama→OpenAIProvider）"]
        PERSONA["personas/<br/>人设注册表（目录式加载）"]
        AUTOM["automation/<br/>croniter 调度"]
        MEM["memory/<br/>语义记忆 coworker.db"]
...
### 2.1 后端包 `coworker/campus/`（11 个文件，全部 L0 纯新增）
...
| `store.py` | `campus.db` 连接管理 + 19 张表建表 + `schema_meta` 版本迁移。**照抄 `coworker/memory/sqlite_store.py:13-43` 的连接模式**（`check_same_thread=False` + `threading.RLock` + 单连接），并补 `PRAGMA journal_mode=WAL` | `class CampusStore: __init__(self, path: str \| Path)`、`def migrate(self) -> int`、`def current_version(self) -> int`、各表 CRUD 方法 | `coworker.secrets.state_dir` |
| `config.py` | campus 偏好读取。**自读** `global_config_path()` 的 TOML（含嵌套 `[campus.*]` 表），不修改 `coworker/config.py`（该文件只认 18 个顶层 key，见 PRD §13-A4） | `def load_campus_config() -> CampusConfig`、`class CampusConfig` | `coworker.config.global_config_path` |
...
| `grading.py` | 结构化批改引擎。**独立于 `coworker/reviewer.py`**（见 ADR-01）。四级降级 L0→L3 | `class GradingEngine: __init__(self, provider, model)`、`async def grade(self, req: GradeRequest) -> GradeResult`、`@dataclass GradeRequest/GradeResult` | `coworker.providers`（`ProviderClient.complete`） |
| `library.py` | 资料导入、按页切片（pypdf 逐页 + `page_no`）、目录路由检索（L1 主力）与关键词召回（L2 补充）。**不引入 embedding**（见 ADR-08） | `class CampusLibrary: __init__(self, store, lib_dir)`、`def import_pdf(self, profile_id, file_path) -> SourceDoc`、`def retrieve(self, profile_id, query, top_k=6) -> list[Chunk]` | `coworker.pdf_support`（`extract_text`） |
...
| `routes.py` | FastAPI `APIRouter`，约 40 个端点。契约详见 `03-API接口设计.md` | `router = APIRouter(prefix="/v1/campus")`、`def build_campus_router(manager) -> APIRouter` | `coworker.server.app`（仅类型引用） |
| `automation_templates.py` | 4 个自动化模板定义（每日复习推送 / 周报复盘 / 冲刺 / 节点提醒），**通过既有 automation CRUD 创建，不新增调度器** | `TEMPLATES: list[AutomationTemplate]`、`def install_template(mgr, profile_id, tpl_id) -> list[str]` | `coworker.server.manager`（task_store） |

**落点**：`coworker/campus/`（与 `coworker/memory/`、`coworker/teams/` 平级）。
...
| 资产 | 路径 | 说明 |
|---|---|---|
| 学习库 | `state_dir()/campus.db` | Windows 11 实际为 `C:\Users\<用户名>\AppData\Roaming\coworker\campus.db`（`coworker/secrets.py:28-44`）。19 张表 |
...
```mermaid
flowchart LR
    subgraph CAMPUS["coworker/campus/  【新增】"]
        S["service.py"]
        G["grading.py"]
        L["library.py"]
        R["routes.py"]
...
| 既有模块 | campus 怎么用 | 边界 |
|---|---|---|
| `providers/` | `grading.py` 等直接持 `ProviderClient`，调 `complete(model=..., messages=..., **settings)`。**结构化输出通过 `**settings` 透传 `response_format={"type":"json_object"}`**（Ollama 走 `OpenAIProvider`，`openai_provider.py:189-193` 原样透传；失败按 `openai_provider.py:92-120` 的 re-raise 行为去掉该参数重试） | 不改 providers 任何文件；不做运行时能力探测（`ModelCapabilities` 无 json_mode 标记，改用静态推荐清单，见 ADR-06） |
...
| `server/app.py` | 唯一改动：`create_app()` 内插入 `app.include_router(campus_router)`（`app.py:187` 的 `app = FastAPI(...)` 之后；该文件 2882 行 180 条内联路由、全项目无 APIRouter 先例，故 campus 自带 router 挂载） | campus 端点全部带前缀 `/v1/campus/*`，与既有 180 条路由零冲突；沿用既有 `X-OpenWorker-Token` 鉴权（`app.py` 的 `_request_authenticated`） |
| `coworker/reviewer.py` | **不调用、不修改**。它是动作安全闸门（`reviewer.py:177, 349-357`），输出契约只有 `allow/deny/unsure`，与批改无关（ADR-01） | 批改引擎只借鉴其工程模式：`asyncio.wait_for` 超时、异常兜底不抛、token 计量 |
| `coworker/teams/` | **V0.1 不调用写路径**。KY-03 降级为自建只读任务列表（ADR-11） | `board_card_id` 字段保留但不写入；V0.2 若做只读同步只调 `GET /v1/board/items` |
| `coworker/compaction.py` | **不依赖**。资料问答走按页切片检索（ADR-08）；Compaction 仍服务于既有会话，campus 不触发它 | — |

### 4.3 对 `coworker.db` 与 `campus.db` 的隔离原则

1. 两个库、两条独立连接、两个独立 store 类；`campus.db` 的连接管理**照抄** `coworker/memory/sqlite_store.py:13-43`（`check_same_thread=False` + `RLock` + 单连接），并补 `PRAGMA journal_mode=WAL`。
...
sequenceDiagram
    ...
    participant PV as providers (OpenAIProvider/Ollama)
...
| 9 | `coworker/server/app.py` | `:187`（`app = FastAPI(...)` 之后） | `app.include_router(build_campus_router(manager))` 单行 | campus 路由挂载 | INF-01 |
...
- **决策**：新建 `coworker/campus/grading.py`，独立调用 `ProviderClient.complete`；**不修改 reviewer.py**——它是权限链路一部分（`config.py:45-54` Auto-Approve/Shadow 依赖），改它会污染安全语义。
- **借鉴**：reviewer 的工程模式——`asyncio.wait_for` 超时、异常兜底不抛、token 计量（`reviewer.py:349-396`）。

### ADR-02 技能包落点改为人设 bundle（对应 §13-A2）

- **背景**：`coworker/skills/campus/` 不会被扫描——`SkillStore` 只认 `state_dir()/skills`（`skills/store.py:87`）与 `<workspace>/.coworker/skills`（`:93, 96-107`）两档。
- **决策**：7 个技能包放 `coworker/personas/builtin/<persona-id>/skills/<skill-name>/SKILL.md`。依据：`personas/registry.py:184-192` 支持人设目录下 `skills/` 同级；`server/manager.py:5830-5843` 的 `persona_skill_scope()` 会把它作为 `extra_skill_dirs` 注入 `SkillLoader`（`skills/base.py:45-48` 按 `<dir>/<name>/SKILL.md` 发现）；`manifest.skills` 字段（`manifest.py:81, 333`）是 allowlist。
...
- **决策**：① 前端语音/登录开关复用 `surfaces/gui/src/flags.ts:8-19` 的 `flag(key, fallback)`（localStorage），追加 `showVoice()`/`showLogin()`，零新依赖且天然可逆；② 后端 `campus.*` 偏好由新增 `coworker/campus/config.py` 自行 `tomllib.load(global_config_path())` 读嵌套表，不碰 `coworker/config.py`。
...
- **背景**：`ModelCapabilities` 仅 `tools/vision/pdf/parallel_tool_calls/streaming` 五字段（`base.py:81-90`），无 json_mode 标记；`matrix.py` 无 Ollama 条目。运行时能力探测不可行。
- **决策**：① `coworker/campus/models.py` 维护 `{任务 → [推荐模型, 最低可用模型]}` 静态清单，UI 明示建议；② 结构化输出以 `provider.complete(**{"response_format": {...}})` 试错（Ollama 走 `OpenAIProvider`，`openai_provider.py:189-193` 透传；`:92-120` 对未列举参数 re-raise，故 try/except 后去参重试）。
...
- **背景**：Compaction 触发阈值 Ollama 上 102,400 token（`compaction.py:23-26, 60-68`），200 页中文教材约 15 万 token 单份超限；且摘要有损（`:391`），与 KY-09"页码引用"验收冲突。全项目无向量/FTS 基础设施。
...
| 离线降级 | 无模型时 AI 功能按钮禁用并提示（PRD §7.6）；静态推荐清单驱动"哪些功能当前模型可用"的判定 |
| 错误处理 | 三原则：不静默失败（扫描件判空即 ADR-09）、AI 结果必标"仅供参考"（PRD §7.6 通用原则）、结构化解析失败必须降级而非报错（ADR-01 的 L0→L3） |
| Windows 11 | `state_dir()` 落 `%APPDATA%\coworker`（隐藏目录，隐私面板用 `explorer.exe` 直开绝对路径）；时间 zone 用 `tzdata`（已在 `pyproject.toml`，Windows 无系统 tz 库）；`once` 任务 `fire_at` 存 ISO 本地时间 + timezone 字段 |
