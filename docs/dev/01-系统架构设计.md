...
### 2.1 后端包 `stealth_study/campus/`（11 个文件，全部 L0 纯新增）
...
| `store.py` | `campus.db` 连接管理 + 19 张表建表 + `schema_meta` 版本迁移。**照抄 `stealth_study/memory/sqlite_store.py:13-43` 的连接模式**（`check_same_thread=False` + `threading.RLock` + 单连接），并补 `PRAGMA journal_mode=WAL` | `class CampusStore: __init__(self, path: str \| Path)`、`def migrate(self) -> int`、`def current_version(self) -> int`、各表 CRUD 方法 | `stealth_study.secrets.state_dir` |
| `config.py` | campus 偏好读取。**自读** `global_config_path()` 的 TOML（含嵌套 `[campus.*]` 表），不修改 `stealth_study/config.py`（该文件只认 18 个顶层 key，见 PRD §13-A4） | `def load_campus_config() -> CampusConfig`、`class CampusConfig` | `stealth_study.config.global_config_path` |
...
| `grading.py` | 结构化批改引擎。**独立于 `stealth_study/reviewer.py`**（见 ADR-01）。四级降级 L0→L3 | `class GradingEngine: __init__(self, provider, model)`、`async def grade(self, req: GradeRequest) -> GradeResult`、`@dataclass GradeRequest/GradeResult` | `stealth_study.providers`（`ProviderClient.complete`） |
| `library.py` | 资料导入、按页切片（pypdf 逐页 + `page_no`）、目录路由检索（L1 主力）与关键词召回（L2 补充）。**不引入 embedding**（见 ADR-08） | `class CampusLibrary: __init__(self, store, lib_dir)`、`def import_pdf(self, profile_id, file_path) -> SourceDoc`、`def retrieve(self, profile_id, query, top_k=6) -> list[Chunk]` | `stealth_study.pdf_support`（`extract_text`） |
...
| `routes.py` | FastAPI `APIRouter`，约 40 个端点。契约详见 `03-API接口设计.md` | `router = APIRouter(prefix="/v1/campus")`、`def build_campus_router(manager) -> APIRouter` | `stealth_study.server.app`（仅类型引用） |
| `automation_templates.py` | 4 个自动化模板定义（每日复习推送 / 周报复盘 / 冲刺 / 节点提醒），**通过既有 automation CRUD 创建，不新增调度器** | `TEMPLATES: list[AutomationTemplate]`、`def install_template(mgr, profile_id, tpl_id) -> list[str]` | `stealth_study.server.manager`（task_store） |

**落点**：`stealth_study/campus/`（与 `stealth_study/memory/`、`stealth_study/teams/` 平级）。
...
| 资产 | 路径 | 说明 |
|---|---|---|
| 学习库 | `state_dir()/campus.db` | Windows 11 实际为 `C:\Users\<用户名>\AppData\Roaming\Stealth Study\campus.db`（`stealth_study/secrets.py:28-44`）。19 张表 |
...
```mermaid
flowchart LR
    subgraph CAMPUS["stealth_study/campus/  【新增】"]
        S["service.py"]
        G["grading.py"]
        L["library.py"]
        R["routes.py"]
...
| `stealth_study/reviewer.py` | **不调用、不修改**。它是动作安全闸门（`reviewer.py:177, 349-357`），输出契约只有 `allow/deny/unsure`，与批改无关（ADR-01） | 批改引擎只借鉴其工程模式：`asyncio.wait_for` 超时、异常兜底不抛、token 计量 |
| `stealth_study/teams/` | **V0.1 不调用写路径**。KY-03 降级为自建只读任务列表（ADR-11） | `board_card_id` 字段保留但不写入；V0.2 若做只读同步只调 `GET /v1/board/items` |
| `stealth_study/compaction.py` | **不依赖**。资料问答走按页切片检索（ADR-08）；Compaction 仍服务于既有会话，campus 不触发它 | — |

### 4.3 对 `coworker.db` 与 `campus.db` 的隔离原则

1. 两个库、两条独立连接、两个独立 store 类；`campus.db` 的连接管理**照抄** `stealth_study/memory/sqlite_store.py:13-43`（`check_same_thread=False` + `RLock` + 单连接），并补 `PRAGMA journal_mode=WAL`。
...
| 9 | `stealth_study/server/app.py` | `:187`（`app = FastAPI(...)` 之后） | `app.include_router(build_campus_router(manager))` 单行 | campus 路由挂载 | INF-01 |
...
- **决策**：① 前端语音/登录开关复用 `surfaces/gui/src/flags.ts:8-19` 的 `flag(key, fallback)`（localStorage），追加 `showVoice()`/`showLogin()`，零新依赖且天然可逆；② 后端 `campus.*` 偏好由新增 `stealth_study/campus/config.py` 自行 `tomllib.load(global_config_path())` 读嵌套表，不碰 `stealth_study/config.py`。
...
- **决策**：① `stealth_study/campus/models.py` 维护 `{任务 → [推荐模型, 最低可用模型]}` 静态清单，UI 明示建议；② 结构化输出以 `provider.complete(**{"response_format": {...}})` 试错（`openai_provider.py:189-193` 透传；`:92-120` 对未列举参数 re-raise，故 try/except 后去参重试）。
...
| Windows 11 | `state_dir()` 落 `%APPDATA%\Stealth Study`（隐藏目录，隐私面板用 `explorer.exe` 直开绝对路径）；时间 zone 用 `tzdata`（已在 `pyproject.toml`，Windows 无系统 tz 库）；`once` 任务 `fire_at` 存 ISO 本地时间 + timezone 字段 |
