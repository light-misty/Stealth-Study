...
> ① `ss/reviewer.py` 是动作安全闸门，`review()` 无 rubric 参数、输出只有 `allow/deny/unsure`（`reviewer.py:177, 349-357`）——批改**完全不经过它**，新建 `grading.py` 独立调 `ProviderClient.complete`（ADR-01），不改 reviewer.py 一行。
...
| 模块 | 职责 | 不负责 |
|---|---|---|
| `ss/campus/rubrics.py` | rubric / 归因枚举的唯一权威常量（05 文档 §5） | 不含任何代码逻辑，纯文本模板字符串 |
| `ss/campus/models.py` | `{任务 → [推荐模型, 最低可用模型]}` 静态清单（ADR-06），决定 grade 请求用哪个模型 | 不做运行时能力探测 |
| `ss/campus/grading.py` | 结构化批改引擎：prompt 组装、模型调用、四级降级、输出校验 | 不落库（落库在 `service.py`）、不检索 |
| `ss/campus/library.py` | 资料导入、按页切片、三级检索、QA 组装 | 不做批改；不改 `pdf_support.py` |
| `ss/campus/service.py` | 编排：C1/E5 端点 → GradingEngine / CampusLibrary → `store.py` 落库（attempt.degrade_level、doc_chunk） | 不直接调 provider |
