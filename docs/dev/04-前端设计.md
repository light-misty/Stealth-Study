...
**（d）`Onboarding.tsx` step 2 的登录提示带（约 `:240-280`，`onboarding.signin_for_oneclick` 文案带 + `ob-cloud-signin` 按钮）**：整块包 `{showLogin() && (...)}`。step 1 的 provider 表单（仅 API Key）**保留**——那是模型配置不是登录（PRD §3.3.3 边界定义），G-06 验收的是"无登录/注册/授权页面"。

不触碰：`api.ts` 的 `cloudLogin/waitForCloudSignIn` 等函数、`AccessSection.tsx`、连接器 OAuth 逻辑（归 Integrations，PRD §3.3.3"不授权也能 100% 用"）。

### 4.9 侵入点 #9 — `stealth_study/server/app.py:187`（后端）
...
```ts
/** StealthStudy v1.1（PRD G-05/G-06）：语音识别与云登录的入口开关。
