...
- 文档编号：PRD-StudyBuddy-v1.0-20260908
- 版本：v1.0（初版）
- 日期：2026-09-08
- 产品经理：许清楚（Xu）
...
仓库内部代号保留 `HIU-WorkSpace` / `coworker`；本文档对外产品名重新设计为「学伴 AI（StudyBuddy）」。
...
```mermaid
flowchart LR
    A[学生向 AI 获取资料与讲解] --> B[AI 产出 讲解/总结<br/>Markdown + 语音]
    B --> C[一键沉淀本地<br/>write_file 落盘 .hiu/notes]
...
| 画像 | 描述 | 典型诉求 |
|------|------|----------|
| 四六级备考生 | 大一至大三，需集中突破听力/阅读/写作/翻译 | 高频词记忆、真题精解、作文批改、听力口语练习 |
| 考研生 | 大三至大四，长线备考（政/英/数/专业课） | 长线冲刺计划、讲义整理、错题沉淀、复试口语 |
...
| B-03 | AI 对话与讲解（Markdown/流式/语音入口） | A | #7 | P0 | 复用 Composer/Markdown/Transcript |
| B-04 | 语音输入（STT 中文/多语种） | C | #12 | P1 | 替换默认英文模型，支持中文/多语种（见 R1） |
| B-05 | 一键沉淀本地（write_file） | C | #13 | P0 | 核心缺口：新增受 workspace 限定写文件工具，对齐 save_skill 预览-确认范式，落盘 `<workspace>/.hiu/notes/` |
...
| B-11 | 备考人设体系 | B | #9 | P0 | 新增 hiu-student-cet / kaoyan / cert 三台人设（见 C1-01/C2-01/C3-01） |
...
| C1-05 | 听力口语练习 | C | #12 | P1 | 依赖 B-04 中文/多语种 STT |
| C1-06 | 每日打卡 + 阶段词汇计划 | B | #10 | P1 | 打卡 cron + 阶段性词表计划模板 |
...
| C2-05 | 口试/复试口语练习 | C | #12 | P1 | 依赖 B-04 |
...
| 离线可用性 | STT 离线 whisper；本地资料检索与复习可离线使用；AI 讲解调用需联网（BYOK 推理在提供商侧，属预期） |

---

## 5. 交互设计规范

### 5.1 复用现有人设/记忆/技能/定时/语音/计划卡 UI 骨架

- **人设选择**：复用 `PersonasTab`，切换三台备考人设。
- **记忆/资料库**：增强 `MemorySection` 为「资料库与复习」视图（B 档）。
- **技能包**：复用 `SkillsTab` 承载备考技能包。
- **定时任务**：复用 `ScheduledView` 做复习提醒/打卡。
- **语音**：复用 `Composer` 语音入口（替换模型后支持中文）。
...
| 人设与技能包 | 本地 persona manifest + skills 文件夹 | hiu-student-* 人设、备考 SKILL.md |
| 落盘资料文件 | `<workspace>/.hiu/notes/` | write_file 落盘的 Markdown 资料 |

---

## 7. 项目里程碑与迭代计划

采用迭代制 I1-I5，时间盒为建议值（非承诺工期），工作量以 XS/S/M/L 表示，不绑定人日与周期。

| 迭代 | 目标 | 关键交付 | 工作量 | 时间盒建议 |
|------|------|----------|--------|------------|
| I1 | 核心闭环打通 | B-01/B-02/B-03/B-05/B-06/B-07/B-11/B-13、三台人设骨架 | L | 约 3 周 |
| I2 | 人设与技能内容 | C1-01~06/C2-01~05/C3-01~04 技能包与词库内容（A/B 配置层） | M | 约 2-3 周 |
| I3 | 复习体验深化 | B-09/B-10 计划与打卡、闪卡/错题重练 | M | 约 2-3 周 |
| I4 | 语音与多语种 | B-04 STT 中文模型替换、C1-05/C2-05 语音练习 | M | 约 2-3 周 |
| I5 | 打磨与分发 | 人设分发纳入 release（R2）、跨平台打包（R5）、Onboarding、B-12 可选 | M | 约 2-3 周 |

```mermaid
gantt
    title 迭代计划 I1-I5（时间盒为建议值，日期仅作示意）
    ...
    section 语音多语种 I4
    STT 中文模型 :i4, after i3, 18d
    section 打磨分发 I5
    release/打包/onboarding :i5, after i4, 18d
```
...
| B-03 | 对话支持流式 Markdown 渲染与语音入口 |
| B-05 | 在对话中对 AI 总结触发「保存」后，经预览-确认落盘至 `<workspace>/.hiu/notes/`，目录自动创建、覆盖有保护 |
| B-06 | 资料库可浏览/检索笔记/错题/单词卡，支持闪卡复习与错题重练 |
| B-07 | 记忆封装为知识卡片，含类型与复习字段，可被 B-06 检索 |
| B-11 | 三台备考人设（cet/kaoyan/cert）可在 PersonasTab 选择并生效，不改 VALID_GROUPS 枚举 |
...
| #10 | 计划卡 / 待办 | B-10 | B |
| #11 | 审计视图 | B-12 | A |
| #12 | 语音输入（STT，英文模型） | B-04（换中文/多语种模型） | C |
| #13 | 本地文件写入（缺失） | B-05 write_file | C |
| #14 | 本地资料检索/复习视图（缺失） | B-06 | C/B |

> 首期功能边界即 C 档三项（写文件、STT 多语种/中文模型、本地资料检索/复习视图）与 B 档四项（记忆封装、人设新增、人设分发、计划模板）。超出此范围的功能一律不纳入。
...
### 11.1 前置改造风险（来自能力盘点第 5 节）

| 编号 | 风险 | 等级 | 说明 |
|------|------|------|------|
| R1 | STT 多语种/中文模型 | 高（功能前置） | 默认英文模型不可用中文；需换模型并做下载与 SHA256 校验，注意体积（142MB+）分发 |
| R2 | 人设分发机制（ships/group/release） | 中 | 新人设如何进入 release 构建，避免仅内部构建可见 |
| R3 | 本地 Python sidecar 启动 | 中 | Tauri 2 稳定拉起 FastAPI+aisuite，首启依赖就绪与失败重试 |
| R4 | BYOK 首次配置门槛 | 中 | 学生无 Key 时首启可能卡住，需 Onboarding 与免费额度模型推荐 |
| R5 | 跨平台打包 | 中 | Win/macOS/Linux 分发，模型与 sidecar 体积管控 |

### 11.2 需用户/校方拍板的阻断级问题

1. **STT 模型选型与分发策略（R1）**：采用哪个中文/多语种 whisper 模型？随包还是按需下载？体积与带宽如何权衡？（阻断级，影响四六级听力口语场景）
2. **新人设分发进入 release 的机制（R2）**：用 `ships:true` + `group:general` 还是调整 release 构建脚本？需明确构建方式。（阻断级，影响三台人设是否对用户可见）
3. **本地 Python sidecar 启动与首启引导（R3）**：跨平台 sidecar 打包路径、启动顺序、失败重试与状态提示方案是否可行？（阻断级，影响应用能否启动）

### 11.3 其他待确认

- 三台备考人设的具体 system_prompt 与技能包 SKILL.md 内容需学科专家/教研侧评审（待确认）。
