# 偷偷学（StealthStudy）— 产品需求文档

> 基于开源项目 **OpenWorker** 的二次开发 PRD
...
- 通过 **Providers 路由** 对接 OpenAI / Anthropic / Gemini / Bedrock / Vertex / Codex；
- 通过 **Tools**（files / search / shell / git / todo / plan / ask / subagent）操作本地与网络；
- 有 Reviewer、PDF 解析、Compaction（长上下文压缩）、权限引擎与审计日志。

一句话：**它是一个"能带团队干活、能定时干活、能记住事、能在本机跑、数据留本地"的 Agent 运行时。**
...
| 需要"数据不外传 / 自带 Key 可控"       | 数据本地存储 + 自带云端 API Key             | **核心竞争力** |
...
> **一句话定位：**
> **偷偷学 = 装在电脑里的本地备考作战室 —— 不登录、数据只存在自己硬盘、模型由你自配的云端 API Key 驱动的四六级 / 考研 / 证书 AI 备考台。**

三条差异化支柱：

1. **数据本地（隐私可控）**：默认数据不出本机；无账号、无云端同步、无学习行为被采集；模型统一通过云端 API Key 调用。
...
| 候选        | 中文名  | 英文名          | 说明                     |
| --------- | ---- | ------------ | ---------------------- |
| **A（推荐）** | 偷偷学  | StealthStudy | 延续 ss 品牌语义，"工友"有陪伴感，易记 |
...
| 维度   | 重构前：偷偷学                                  | 重构后：偷偷学（StealthStudy）                          |
| ---- | ---------------------------------------- | ---------------------------------------------- |
...
| 语音输入 | 预留（语音转文字，当前关闭）                           | **关闭并隐藏**（代码保留，flag 关闭）                        |
| 数据边界 | 本地 + 可选云端连接器                             | **默认 100% 本地**；模型调用需联网（云端 API Key）；资料检索联网可关    |
...
- **对产品的要求**：低成本用（自带云端 Key），界面中文，不要社交打卡，不要打卡排行榜带来的焦虑。
...
- **设备**：一台游戏本（16GB + RTX 4060），愿意为效率工具付费，配过云端模型 Key。
- **典型场景故事**：
...
| ------------- | -------------------------------- | ----------------------------- | ------------------------------------------------------------------ | --------------------------------------------------------------------------- |
| **背单词 App**   | 百词斩、墨墨背单词、不背单词、扇贝单词              | 词汇记忆算法成熟（艾宾浩斯/个性化预测）、词库全、打卡生态 | 只覆盖"词汇"一个维度；错题/作文/听力/规划全无；数据在外（隐私弱）；数据绑账号                          | **不是竞品，是被集成的对象**：本产品做词表调度时，算法思路参考墨墨/扇贝，但不重复造轮子（V0.1 用简化的 SM-2 间隔重复，不追求算法领先） |
| **考研机构 App**  | 新东方、文都、海文等                       | 课程体系、师资、真题库、院校库               | 重内容轻工具；计划靠班主任；资料与批改在体系内闭环，无法导入自己的资料；多数需付费                          | **差异化点**：本产品不提供课程，只做"个人备考操作系统"，且能处理用户**自己的**资料                              |
| **通用 AI 对话**  | ChatGPT / Claude / DeepSeek / 豆包 | 答疑、批改、规划都能做                   | ① 无持久记忆（每次重来）；② 无固定评分标准（批改风格飘）；③ 无结构化数据（错题不落表）；④ 资料上传有隐私顾虑；⑤ 模型走云端 | **核心替代对象**：本产品 = 通用 AI + 持久化 + 结构化 + 数据本地化 + 备考语义封装                         |
...
| **AI 学习助手新品** | 各类"AI 学习机 / AI 自习室"              | 硬件捆绑、督学、机构运营                  | 偏 K12 与机构场景；价格高；非本地；不面向大学生自主备考                                     | 本产品聚焦**大学生自主备考 + 数据本地 + 零成本（自配云端 Key）**                                     |

**市场空白总结（本产品的四个机会点）：**

1. **"自己的资料" + "持久记忆" + "数据本地"三者同时满足** —— 纯云端 AI 难做到数据本地化，本地笔记软件做不到 AI 批改，背单词 App 做不到自有资料。
...
| #  | 假设                                          | 若假设不成立的应对                                                  |
| -- | ------------------------------------------- | ---------------------------------------------------------- |
| A1 | 大学生愿意为了"数据本地 + 零登录"接受**自行配置云端 API Key** 的成本 | 提供"一键检查 + 分步指引 + 模型能力矩阵"；若门槛过高，则需提供内置免费额度（会破坏数据本地定位，需重新决策） |
...
| 需求          | 处置                     | 具体做法                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | 标记                  |
| ----------- | ---------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------- |
| **A. 语音识别** | **关闭并隐藏**，**只需 2 个卡点** | ①【新增】前端开关 `showVoice()` = `flag("voice", false)`，追加在既有 `surfaces/gui/src/flags.ts:8-19`（`flag(key, fallback)` 读 localStorage；`showPersonas()` 是已上线先例）—— **零新增依赖、连配置文件都不用动、天然可逆**②【隐藏/关闭·卡点 1】`SettingsView.tsx:95` 的 tab 过滤：`SET_TABS.filter(tab => tab.key !== "voice")`（**本仓库已有同款先例**：`.filter((tab) => tab.key !== "personas")`）③【隐藏/关闭·卡点 2】`Composer.tsx:733-757` 的麦克风按钮短路（这是**唯一功能入口**；其余语音代码全挂在 `dictation?.recording` / `voiceReady` 之下，录不了就没有后续）④【保留】Rust 侧 `stt/` crate（`ss-stt`, whisper-rs + cpal）**保留在仓库中**，不参与默认构建 / 不初始化 / 不下载模型；通过 Cargo feature `enable-stt`（默认 off）控制⑤【保留】`Composer.voice.test.tsx` 等测试文件保留（改测 flag 关闭路径，或标注 skip）⑥【保留】i18n voice 文案保留（不删 key，只是无入口渲染） | 【隐藏/关闭】+【新增】flag 函数 |
| **B. 登录功能** | **完全去除（用户可感知层面）**      | ①【新增】前端开关 `showLogin()` = `flag("login", false)`（同上，追加在 `flags.ts`）②【隐藏/关闭】`CloudSignIn.tsx`、`AccessSection.tsx` 的登录/账号区由 flag 短路③【隐藏/关闭】`Onboarding.tsx` 中的登录/注册步骤跳过④【隐藏/关闭】`Sidebar.tsx:1242` 附近 `cloud?.signed_in` 账户区不渲染⑤【隐藏/关闭】Connectors 的 OAuth 入口默认折叠且不展示"登录"字样；连接器整体归入 Integrations，核心学习链路不依赖⑥【保留】`tauri.ts` / `api.auth.test.ts` 等文件与函数保留⑦【保留】i18n 文案保留                                                                                                                                                                                                                                                                                                                        | 【隐藏/关闭】+【新增】flag 函数 |
...
| 模型调用                                                | 需要**用户自己的云端 API Key** | **这不是本产品的"登录"**：Key 只存在本机配置中，不经过本产品任何服务器                          |
| 联网搜索（查报名时间等）                                        | 不需要登录本产品              | 走既有 `search` 工具                                                   |
| Cloud 连接器 / Gmail / Slack / Github / Calendar OAuth | **可选**                | 默认折叠在 Integrations 内；**不授权也能 100% 使用学习功能**；本产品不提供"用第三方账号登录偷偷学"的路径 |
| 云同步 / 多端同步                                          | **不做**                | 见 §4.9                                                            |

**判定规则（供架构师执行）**：任何需要"偷偷学自有账号体系"的功能一律不做；任何"用户拿自己的凭据去接第三方服务"的功能，保留但必须满足"不授权 = 功能完全可用"。
...
```mermaid
flowchart TD
    ROOT["偷偷学 StealthStudy<br/>数据本地 · 零登录 · Agent 驱动"]
...
- **页面有什么**：应用标题、窗口标题、关于页展示新产品名（默认"偷偷学 StealthStudy"，可在配置中改回）。
- **点击后发生什么**：无交互，仅展示。
...
- **验收标准**：
  1. **一致性（分模型承诺，v1.1 修订）**：在**推荐云端模型**上，同一篇作文提交两次，分项得分**波动不超过 1 档**；弱模型场景改为"同一篇连批 3 次，取中位数展示，且 UI 标注『评分仅供参考』"（**不承诺 1 档**）；
...
- **⚠️ 归因交互模式（v1.1 修订）**：
  - **V0.1：`attribution`** **默认** **`pending`，AI 给出建议标签，用户一键确认或修改。** 不做全自动落库。理由：五分类在弱模型上稳定度差（架构师 §13.3 B2），且"全自动 + 高置信度"需要**未校准的置信度**（让模型自报 0.82 不可用）。
...
- **验收标准**：
  ...
  4. 同一份答案重复提交两次，得分点判定一致（**推荐模型上**；弱模型 取 3 次中位数，同 CET-09 约定）。
...
| G-04 | G3 隐私   | 数据目录展示 + 模型供给说明                                                                                                 | P1  | 配置 + providers                                         | 中              | 未配置模型时云端调用被明确禁用并提示                                  | 【新增】         |
...
| CET-09 | CET4 | 作文批改（分项得分 + 错误清单 + 升格示范） | P0  | **`campus/grading.py`** + Skills | **高+**（v1.1 上调，拆"批改引擎"与"CET 评分标准"两个子任务） | 推荐模型上两次分差 ≤1 档；弱模型 连批 3 次取中位数 + UI 标注 | 【新增】 |
...
| 2  | **用户注册 / 登录 / 账号体系 / 云同步 / 多端同步**  | 用户明确要求去除；数据本地定位                                     |
...
| 异常         | 表现       | 处理                                                              |
| ---------- | -------- | --------------------------------------------------------------- |
| 未配置任何模型    | 定级测评点不动  | 引导到设置页配置云端 Key，并展示**静态功能可用清单**（哪些功能无模型也能用，v1.1：不再承诺"能力矩阵"运行时检测） |
...
| 定级测评单题提交        | ≤ 2 秒（云端模型）                | 弱网络场景需展示进度条而非转圈                                        |
| 作文批改（800 词）     | ≤ 15 秒（云端）                 | 超时给出"模型响应慢，可切换模型"提示                                    |
| 资料解析（100 页 PDF） | ≤ 60 秒，后台异步，不阻塞 UI         | 展示解析进度                                                 |
| 资料问答（10 轮）      | 单轮 ≤ 10 秒（云端）/ ≤ 30 秒（弱模型） | **切片检索**（L1 目录路由只送相关页，非全文入上下文）；v1.1 修订，不再依赖 Compaction |
...
| 级别                 | 触发条件       | 做法                                                                                                   |
| ------------------ | ---------- | ---------------------------------------------------------------------------------------------------- |
| L0 强约束             | 云端强模型      | `response_format=json_object` + 完整 JSON Schema 示例 + `temperature=0`                                  |
| **L1 填空式（弱模型主路径）** | 弱模型        | **不让模型自由生成 JSON**。给它一个已填好键名的模板（`内容分: ___\|结构分: ___\|错误1原文: ___` …），用行/标记解析（正则 + 分隔符），缺字段用默认值补齐而非整体失败 |
| L2 逐维度拆分           | L1 仍不稳定    | 一次调用只问一个维度（"只输出内容分，0-15 的整数"）。调用次数 ×3，但每次输出极短，弱模型 稳定度大幅提升                                            |
...
| 主观题批改、AI 讲解、AI 出题、定级测评 | ⚠️ 需云端模型 | 无模型配置时，按钮禁用并提示原因                                                   |
| 联网搜索（报名时间核对）           | ❌ 不可用    | 明确提示"未联网，无法核对报名时间"                                                 |
...
| 模型服务不可用          | 提示"模型服务不可用或 Key 无效"，引导到设置页                                       |
| 网络不可用            | 提示"网络不可用，已切换为仅本地数据功能"                                            |
...
> **MVP 标准：一个没考过的二本大二学生，装上后不配置任何账号（配一个云端模型 Key），能在 30 分钟内完成四六级定级测评、看到自己的分数差距、拿到一份到考试日的每日计划，并且第二天晚上 8 点在应用内收到复习卡。**
...
| 版本                | 主题                      | 范围                                                                                                                                                                                      | 关键交付                                                                                                                  | 建议周期    |
| ----------------- | ----------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------- | ------- |
| **V0.0（技术验证，新增）** | **先验证承重墙，再排期**          | 见 §13.6 / §13-A 类结论                                                                                                                                                                     | ① 真实云端模型跑 20 篇作文，量 L0/L1/L2 三档 JSON 解析成功率与批改一致性；② 中文 PDF 按页切片召回 spike；③ `app.include_router` 冒烟                       | **1 周** |
| **V0.1（MVP）**     | **三个备考台同时上线（用户已拍板不分期）** | 全局层 P0 + CET / KY / CERT **全量 P0**（共 34 条）内部按优先级分层推进：**CET 先行**（定级→目标拆解→计划→高频词→作文批改）→ **KY 跟进**（四轨计划→数学轨→资料问答→周报）→ **CERT 收尾**（知识点树→题库刷题→主观题批改→错题归因→节点提醒）；三组共享统一错题本 / 间隔复习调度 / 4 个自动化模板 | · 语音/登录彻底隐藏（收敛至 2 个卡点，见 §3.3.1）· 三台入口均开放，首屏聚焦"今天学什么"· 统一错题本 + 每日复习推送 + 报名节点应用内提醒· `campus.db`（19 张表）与 6 人设 / 7 技能包    | 6–8 周   |
| **V0.2**          | **深化差异化与体验**            | P1 中优先级最高的部分                                                                                                                                                                            | · CET 计时模考（模拟收答题卡）+ 听力精练 + 翻译批改· KY 落后≥15% 预警 + 专业课看板只读同步· CERT 变式题生成 + rubric 强化 + 归因分布图· 视觉模型兜底（扫描件无文字层场景）· 资料库检索增强 | +4–6 周  |
| **V0.3**          | **打磨与开放能力**             | 其余 P1 + 选定 P2                                                                                                                                                                           | · 数据导出/导入、备份到指定文件夹（缓解 P6）· 更多证书类型（教资/NCRE 之外的预留枚举）· 性能打磨与多模型适配                                                        | +4 周    |
| **V1.0**          | **稳定版**                 | 全部 P0 + 选定 P1 + P2（视反馈）+ 测试补齐 + 性能优化                                                                                                                                                    | · 三台完整能力· Vitest 覆盖对齐既有水平· 多模型适配验证· 中英双语完整· 隐私面板与一键清除                                                                 | +3–4 周  |
...
| <br />   | 单用户周均学习时长             | 任务实际耗时之和                           | ≥ 4 小时                                                                                     |
| <br />   | 自动化提醒打开率              | 推送被点击 / 推送总数                       | ≥ 40%                                                                                      |
| <br />   | **逾期节点被看见率**（v1.1 新增） | 节点到期后 7 天内被打开应用看到的比例（分活跃/低活跃用户群观测） | 活跃用户 ≥ 90%；低活跃用户单独观测 —— **该指标直接检验 D2（应用内提醒）决策的盲区，若低活跃用户错过率显著偏高，触发 §11.2 Q13 重新评估 OS 通知立项** |
| **学习效果** | 定级/模考估分提升             | 两次测评/模考估分差                         | 四级用户 8 周内 +30 分                                                                            |
| <br />   | 错题再错率                 | 复习队列中再次答错的比例                       | 逐轮下降，第 3 轮 ≤ 20%                                                                           |
| <br />   | 考纲覆盖率（证书）             | 已掌握节点 / 总节点                        | 考前 ≥ 90%                                                                                   |
| <br />   | 主观题批改次数               | 人均累计                               | ≥ 8 次 / 备考周期                                                                               |
| **留存**   | 次周留存                  | 本周活跃用户中下周仍活跃                       | ≥ 60%                                                                                      |
| <br />   | 备考周期完成率               | 走到考试日仍 active 的档案比例                | ≥ 50%                                                                                      |
| <br />   | 跨考试复用率                | 使用 ≥2 个备考档案的用户比例                   | ≥ 35%                                                                                      |
| **性能**   | 冷启动到可交互               | —                                  | ≤ 3 秒                                                                                      |
| <br />   | AI 任务 P90 耗时（云端）      | 批改/讲解                              | ≤ 20 秒                                                                                     |
| <br />   | 崩溃/异常率                | 每千次会话                              | ≤ 5                                                                                        |
| **信任**   | 一键清除使用率               | —                                  | 不作为正向指标，但需可观测                                                                              |
| <br />   | 纯本地数据功能占比             | 纯本地数据功能 / 总功能                      | ≥ 60%                                                                                      |
...
| #  | 风险                                                                                                                                                             | 影响                   | 概率 | 缓解措施                                                                                                                                                                                                                                                                                                |
| -- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------- | -- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| T1 | **弱模型能力不足**：作文批改、错题归因、知识点抽取在弱模型上质量差，甚至无法输出结构化 JSON                                                                                                             | 高 —— 直接决定"差异化"卖点是否成立 | 高  | ① 用**静态推荐清单**（按模型族硬编码能力，因 `capabilities.py` 仅有 5 字段且无 `json_mode` 标记、`matrix.py` 未覆盖全部模型条目，无法运行时自检），并通过向 OpenAI 兼容端点传 `response_format={"type":"json_object"}` 试错探测；② 为弱模型设计"填空式 prompt + 标记解析"四级降级（L0 强约束→L1 填空模板→L2 逐维度拆分→L3 弃结构化），见 §13-A2；③ 在 UI 明示"该功能建议使用 XX 及以上模型"；④ 关键功能（批改）默认走云端，弱模型仅做可选降级 |
| T2 | **检索层从零建且中文效果未知**：全项目无任何检索基础设施（无 FTS5、无 embedding，已 grep 确认）；中文无空格分词，FTS5 unicode61 对中文效果差；Compaction **不能**替代（触发阈值在中等上下文模型上仅 102,400 token，且摘要会丢页码，见 §3.1-10） | 高 —— KY-09 是考研台 P0   | 中  | ① **已改方案**：按页切片 + 两级检索（L1 目录路由 / L2 bigram）；② **V0.0 里程碑先做中文召回 spike**（§8.2），召回重合率不达标不排期；③ 不引入 embedding 依赖；④ Compaction 降级为"单页精讲"等小上下文场景的可选增强                                                                                                                                                      |
...
| T5 | **批改一致性**：同一篇作文两次批改分差大                                                                                                                                         | 中 —— 直接影响用户信任        | 中  | ① 评分标准写进技能包（**官方五档**/分项 rubric，14/11/8/5/2 分档）；② 温度设低；③ 输出必须含得分点级证据；④ **分模型承诺**（v1.1）：验收"波动 ≤1 档"只在云端强模型上承诺，弱模型改为连批 3 次取中位数 + UI 标注"评分仅供参考"；⑤ INF-11 评测脚手架持续量测                                                                                                                                      |
...
| #  | 风险                              | 影响         | 概率 | 缓解措施                                                                                                    |
| -- | ------------------------------- | ---------- | -- | ------------------------------------------------------------------------------------------------------- |
| P1 | **配置模型 Key 的门槛劝退用户**（假设 A1）     | 高          | 中  | ① 首次启动的分步引导（推荐配置云端 Key）；② 明确"无模型也能用"的功能清单（L0–L2 层全部功能，见 §8.1 分层）；③ §3.1-5 静态推荐清单 + UI 明示"建议使用 XX 及以上模型" |
...
| D3 | **是否插入 V0.0 技术验证里程碑？**               | **插入，1 周**。理由：AI 批改链路（`grading.py`）是三台共同地基、"差异化"卖点的承重墙，而它目前建立在两个都未验证的前提上（弱模型结构化输出、中文 PDF 召回）。V0.0 内容：① 真实云端模型跑 20 篇作文，量 L0/L1/L2 三档 JSON 解析成功率与批改一致性；② 中文 PDF 按页切片召回 spike；③ `app.include_router` 冒烟。V0.0 结论直接决定 L3/L4 的验收标准与排期                             | 2026-09-08 | §8.2                |
...
| D7 | **模型供给方式？**                          | **用户自配云端 API Key**，不做内置免费额度（用户确认）。批改等 AI 功能在无模型时明确禁用并列出可用功能清单                                                                                                                                                                                               | 2026-09-08 | §7.2、Q3             |

### 11.2 仍待确认问题

| #       | 问题                                                                                 | 选项                                                      | 建议                                                            | 阻塞级别   |
| ------- | ---------------------------------------------------------------------------------- | ------------------------------------------------------- | ------------------------------------------------------------- | ------ |
| **Q1**  | 产品名用什么？                                                                            | A 偷偷学 StealthStudy / B 备考台 PrepDesk / C 学霸工坊 StudyForge | **A**（延续 ss 品牌，且"工友"贴合陪伴感）                                    | 低（可后置） |
| **Q2**  | ~~语音与登录的处置是否接受"Flag 屏蔽 + 代码保留"？~~ **【已决策 D6】**                                     | —                                                       | Flag 屏蔽保留代码                                                   | 已解决    |
| **Q3**  | ~~是否接受"必须自配云端 Key 才能批改/讲解/规划"？~~ **【已决策 D7】**                                      | —                                                       | 接受                                                            | 已解决    |
...
1. 新东方四六级网《四级考试满分710分，各题型分值分布一张图看懂》（2026-08）— <https://cet4-6.xdf.cn/202608/15364321.html>
   - 引用于：写作 106.5(15%) / 听力 248.5(35%) / 阅读 248.5(35%) / 翻译 106.5(15%)；各题型单题分值；425 分目标拆解（听力 \~149 / 阅读 \~149 / 写译各 \~70）
2. 新东方四六级网《四六级通过率到底是多少？真实数据告诉你答案》（2026-07）— <https://cet4-6.xdf.cn/202607/15302020.html>
   - 引用于：四级通过率约 40%–45%、六级约 30%–35%（机构估算）；985 高校四级可达 70%+；听力得分率不足 50%；裸考通过率不足 20%
3. 新东方教育头条《四六级通过率到底是多少？全国平均数据院校差异分析》（2026-04）— <https://mtoutiao.xdf.cn/cet4-6/202604/15157862.html>
   - 引用于：官方不公布统一通过率；2024 年四级平均通过率 42.7%（据称来自官方白皮书，**未获证实**）
4. AcademicJobs《2025下半年CET四六级成绩查询：完整指南》（2026）— <https://www.academicjobs.com/higher-education-news/2025-cet227-academicjobs-6383>
   - 引用于：CET 年报名超 2000 万、2024 下半年约 1100 万（**未获官方证实**）；另一套通过率口径（四级 25–35%、六级 10–20%）—— **与来源 7 冲突，已在正文标注**
5. 新东方考研网《2026考研大数据出炉：报名人数、过线率、调剂成功率全解析》（2026-07）— <https://kaoyan.xdf.cn/202607/15259032.html>
   - 引用于：2026 报名 343 万（较 2025 年 388 万降 45 万）；过线率 31.7%；统考录取率约 14%；调剂成功率约 18%
6. 今日头条《26考研报名人数下降50万，为何985分数线不降反升？》（2026）— <https://m.toutiao.com/article/7571750268842082816>
   - 引用于：**与来源 10 冲突的"2026 年 388 万"口径**；往届生占比超 55%
7. 新东方考研网《27考研科目与分数全解析》（2026-05）— <https://kaoyan.xdf.cn/202605/15180255.html>
   - 引用于：政治题型与分值；英一/英二结构差异；数一/二/三 内容占比；300 分制与 500 分制
8. 天任教育《考研公共课与专业课全解析》（2026）— <https://m.tianrenedu.com.cn/changshi/728948.html>
   - 引用于：统考专业课 300 分科目（333/312/313/306）与 150 分科目（408/199）
9. 搜狐/今日头条 援引《中国教育考试年报》关于教资的数据（2025–2026）
...
10. 教育界杂志社《2026国内英语单词记忆赛道排名及市场格局》（2026）— <https://m.jyjzzs.com/view.php?aid=175355>
    - 引用于：竞品格局（百词斩、墨墨、扇贝、不背单词等）
11. 网易《背单词软件推荐 2026｜主流背单词 app 优缺点对比》（2026）— <https://www.163.com/dy/article/L5DOOMHP0536M5ZB.html>
    - 引用于：各背单词 App 的优缺点与适配人群
12. 搜狐《挑抗遗忘的背单词App，只要看这四个指标》（2026-09）— <https://learning.sohu.com/a/1068007358_122542400>
...
> **评审日期**：2026-09-08
> **基线代码**：`D:\DeskTop\HIU-WorkSpace`（偷偷学 / ss）
...
#### B2. 弱模型能否稳定输出结构化 JSON？

> ## 结论：**语法层面可以（有可行手段），语义/schema 层面在弱模型上不可靠。**
>
> ## 产品建议：**弱模型批改可以做，但必须先降预期 —— V0.1 定位为"可用但有损"，默认仍推荐云端强模型；验收标准"两次分差 ≤1 档"不应在弱模型上承诺。**
...
- `capabilities.py` 是**静态启发式表**（按 model 名前缀推断），不是运行时探测；`matrix.py:40-52` 的 `ModelEntry` 只有 `label / caps / context_window`，**且表里未覆盖全部模型条目**。
- 模型能力分支（`capabilities.py:28-33`）只判断 `tools=True, vision=按名字匹配 -vl/vision/llava..., parallel_tool_calls=False, streaming=True`。

→ **PRD §7.1"用既有 capabilities.py 与 matrix.py 判断当前模型是否支持结构化输出"与 §10.4 相应依赖行，不成立。** 已在原处修正。

**（2）好消息：有一条可行且零侵入的结构化输出通道。**

- 模型调用走 **`OpenAIProvider`** 兼容层：`providers/registry.py` 的各 `_build_*` 构造器返回 `OpenAIProvider`（OpenAI 兼容的 `/v1`）。
- `OpenAIProvider.complete`（`openai_provider.py:181-193, 204`）把 `**settings` **原样透传**给 `client.chat.completions.create(**kwargs)`。
- → `grading.py` 可以传 `response_format={"type": "json_object"}`（各 OpenAI 兼容 `/v1/chat/completions` 支持），**语法上保证是合法 JSON**。
...
- 维度名正确（`内容/结构/语言` 还是 `content/structure/language`？弱模型 会自由发挥）；
- 错误清单完整（漏条、重复条）；
...
| ------------------ | ---------- | ----------------------------------------------------------------------------------------------------------------------------- |
| L0 强约束             | 云端强模型      | `response_format=json_object` + 完整 JSON Schema 示例 + `temperature=0`                                                           |
| **L1 填空式（弱模型主路径）** | 弱模型        | **不让模型自由生成 JSON**。改为给它一个**已填好键名的模板**，让它只填值：`内容分: ___\|结构分: ___\|语言分: ___\|错误1原文: ___` … 用行/标记解析（正则 + 分隔符），**缺字段用默认值补齐而非整体失败** |
...
| → 中等上下文模型实际触发点  | **102,400 tokens**                          | 计算得出                         |
...
3. **10 轮追问的成本**：每轮都把整份资料带进上下文，10 轮 = 10 次 15 万 token 的 prefill。在弱模型上是分钟级延迟，PRD §7.1"单轮 ≤10 秒"不可能达成。
...
- 但两个限制：`RASTER_MAX_PAGES = 100`（`pdf_support.py:31`，**200 页只能处理前 100 页**）；视觉模型仅当模型名含 `-vl|vision|llava|bakllava|cogvlm|minicpm-v` 等时可用（`capabilities.py:28-33`）。
- 现实性：`qwen2.5-vl`（云端视觉 API）调用成本高、速度慢；但**一页一图，成本高、速度慢**，只适合"关键几页精读"，不适合 200 页全量。
...
| 通知实现  | `server/manager.py:4957-4990` `_notify_task_done()` | 只有两条路：① `broadcast_session()` **WebSocket 推给已打开的 GUI**；② `notify_target` 走**连接器**（Telegram/Slack，需 bot\_token）                   |
...
3. **每次触发会真的跑一个 Agent**（`manager.py:4938-4944` `engine.run(opening)`）—— 即使只是提醒，也会发起一次模型调用，耗时**十几秒到几分钟**，且耗电。
...
**荣誉提名（未进前五但需留意）**：`KY-02 四轨阶段计划生成`（评"高"，实际需 4 套独立 prompt + 异构校验，且验收标准"考数二+英二 vs 不考数学生成的计划结构明显不同"需要对比测试）；`G-03 档案切换器`（评"中"，但"至少 2 个档案不串数据"意味着**每个查询都要带 profile\_id**，是横切关注点，容易漏）。
...
| B3 | **CET-09** 验收 1  | "同一篇作文提交两次，分项得分波动不超过 1 档"                | 加限定：**"在推荐云端模型上"**；弱模型场景改为"连批 3 次取中位数 + UI 标注'评分仅供参考'"                                                                                                             |
...
1. **阶段 0–2 完全不依赖模型**，可在无模型配置的情况下开发、测试、演示 —— 这直接支撑 PRD §7.2"数据本地可用"与假设 A1 的兜底（"无模型也能用的功能清单"）。
2. **阶段 1（减法）放在阶段 2 之前**：它是全 PRD 里唯一"改完立刻能被用户验证"的部分（G-04 的 7 条验收标准全部可测），且技术风险近乎为零。
3. **阶段 3 的** **`grading.py`** **是全局瓶颈**：CET-09/10、CERT-07/14、KY-05/06、CERT-08 全部挂在它上面。**必须先做，且必须先做 B2 的降级验证**（用真实云端模型跑 20 篇作文，量出 JSON 解析成功率），再决定其余需求的验收标准。
...
| 里程碑               | 内容                                                                                               | 建议周期    |
| ----------------- | ------------------------------------------------------------------------------------------------ | ------- |
| **V0.0（技术验证，新增）** | ① 用真实云端模型跑 20 篇作文，量出 L0/L1/L2 三档的 JSON 解析成功率与批改一致性；② 中文 PDF 召回 spike；③ `app.include_router` 冒烟验证 | **1 周** |
...
2. **§8.2 已插入 V0.0 技术验证里程碑（1 周）**，先于 V0.1 排期，规避"批改链路做完才发现弱模型上不可用"的风险。
...
| #  | 对应评审项                 | 正文修订                                                                                                                                                                                                                                                            | 章节                         |
| -- | --------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------- |
| 1  | A1 / B1               | §3.1-7 整行重写：Reviewer 定性改为"动作安全闸门，不可用于批改"；批改改为新建独立链路 `campus/grading.py`；G-15 / CET-09/10/14、CERT-05/07/14、KY-05/06/07 的"复用 Reviewer"全部改为 `campus/grading.py`；CET-09 验收 1 改为**分模型承诺**（推荐模型 ≤1 档；弱模型连批 3 次取中位数 + UI 标注）；新增 §0.2"禁止修改 reviewer.py"红线与 INF-11 评测脚手架 | §0.2、§3.1、§4.3–4.5、§4.8    |
...
| 3  | A3 / B2 / §13.3 B3    | §3.1-10 重写：Compaction 定性改为"事后压缩、中等上下文模型阈值 102,400 token、摘要丢页码，不能承重"；KY-09 改为**按页切片 + 两级检索**（L1 目录路由 / L2 FTS5-bigram），新增召回重合率 ≥60% 验收与"不达标不排期"前置条件；新增 INF-10、`doc_chunk` 表；§7.1 资料问答指标改为切片检索口径                                                                  | §3.1、§4.4、§6.2、§7.1、§4.8   |
...
| 3 | §5.1 流程图："创建 Teams 看板列与卡片"→"创建自建备考看板列与卡片（plan\_task.status 驱动）"                                                                                            | §5.1     |
...
3. **G-15 批改链路的工时仍是估算**：L0–L3 四级降级 + 7 个技能包 rubric 撰写 + 评测脚手架（INF-11），即使 V0.0 给出数据，仍是全项目最大单点。若 V0.0 显示弱模型上 L1 成功率 <70%，建议把"弱模型批改"从 P0 降为 P1（云端批改保 P0），这个决策留到 V0.0 后做。
