...
const PERSONAS = {
  internal: true,
  personas: [
    { id: "cowork", name: "HIU WorkSpace", icon: "cowork", tagline: "Produce a deliverable — research, analysis, scripts", requires_folder: false, builtin: true, tools: ["files", "search"], enabled: true, surfaced: true, default: true, ships: true, group: "general" },
...
const PINNED_SESSION = {
  ...
  workspace: "/Users/test/HIU WorkSpace/launch-note",
  agent: "cowork",
  model: "anthropic:claude-opus-4-8",
  mode: "interactive",
...
const OPS_SESSION = {
  session_id: "ops-1",
  title: "Ops triage",
  workspace: "/Users/test/HIU WorkSpace/ops-triage",
  agent: "ops",
  model: "anthropic:claude-opus-4-8",
  mode: "interactive",
...
const PRIMARY_ROOT = { path: "/Users/test/HIU WorkSpace/launch-note", writable: true, label: "scratch", primary: true, exists: true };
...
export async function mockApi(page: import("@playwright/test").Page) {
  ...
  const projectNames: Record<string, { name: string; key: string }[]> = {
    memory: [
      { name: "hiu-workspace", key: "/k/hiu-workspace" },
  ...
  const boardPayload = () =>
    boardItems.length
      ? { space: "/Users/test/HIU WorkSpace/launch-note", name: "launch-note", items: boardItems }
        ...
        if (/board wake/i.test(msg.text)) {
          send("turn_start", {
            source: {
              connector: "board",
              kind: "channel",
              channel_id: "/Users/test/HIU WorkSpace/launch-note",
  ...
  await page.route("**/v1/**", async (route) => {
    ...
    if (/\/v1\/sessions\/[^/]+\/artifacts$/.test(p)) {
      return json({
        artifacts: [
          {
            path: "reports/security-review.html",
            abs_path: "/Users/test/HIU WorkSpace/launch-note/reports/security-review.html",
    ...
    if (p.endsWith("/v1/cloud/login") && m === "POST") {
      Object.assign(CLOUD_STATE, { signed_in: true, account: "rohit@hiu-workspace.com", user_id: "usr_e2e" });
      return json({ ok: true });
    }
    ...
    if (/\/v1\/connectors\/[^/]+\/connect-managed$/.test(p) && m === "POST") {
      ...
      if (p.includes("/connectors/outlook/")) {
        outlookState.accounts.push({
          account_id: `mbx${outlookState.accounts.length + 1}@hiu-workspace.com`,
          name: `mbx${outlookState.accounts.length + 1}@hiu-workspace.com`,
