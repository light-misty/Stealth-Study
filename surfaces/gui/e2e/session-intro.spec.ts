...
test("ready rows reveal Start → on hover and prefill the composer", async ({ page }) => {
  ...
  await hs.click();
  await expect(page.getByPlaceholder(/Ask your study partner/)).toHaveValue(/HubSpot leads/);
  ...
  await gh.click();
  await expect(page.getByPlaceholder(/Ask your study partner/)).toHaveValue(/weekly progress report/);
});
