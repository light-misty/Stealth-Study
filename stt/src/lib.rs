...
/// A reasonably fast English model for short Stealth Study prompts (~142 MB).
...
fn build_stream(
    ...
    let on_error = |error| eprintln!("[stealth-study-stt] microphone stream error: {error}");
    match sample_format {
...
mod tests {
    ...
    fn readiness_requires_a_verified_model_and_persists_after_a_test() {
        let unique = SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let dir = std::env::temp_dir().join(format!("stealth-study-stt-readiness-{unique}"));
