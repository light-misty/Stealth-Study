...
def _browser_page(
    ...
    return (
        ...
        f"<title>{_html.escape(title)} — StealthStudy</title><style>"
        ...
        "</style></head><body>"
        '<div class="card"><div class="mark"><i></i>StealthStudy</div>'
        f"{icon}<h1>{_html.escape(title)}</h1><p>{_html.escape(detail)}</p>{err}</div>"
        '<div class="foot">Served locally by StealthStudy on your Mac</div>'
        "</body></html>"
    )
...
_CONNECT_FAILED_DETAIL = (
    ...
    "Close this tab and try again from StealthStudy."
)
...
def create_app(manager: SessionManager) -> FastAPI:
    ...
    async def require_sidecar_token(request: Request, call_next):
        ...
        return JSONResponse(
            {"error": "missing or invalid StealthStudy sidecar token"},
            status_code=401,
        )
    ...
    async def mcp_oauth_callback(
        ...
        if error:
            return HTMLResponse(
                _browser_page(
                    "Sign-in failed",
                    "The service reported an error. Return to StealthStudy and try again.",
                    ok=False,
                    error=error,
                ),
                status_code=400,
        ...
        if not code or not mcp_oauth.deliver_callback(code, state or None):
            return HTMLResponse(
                _browser_page(
                    ...
                    "The sign-in may have timed out. Return to StealthStudy and start it again.",
                    ok=False,
                ),
                status_code=400,
            )
        return HTMLResponse(
            _browser_page(
                "Connected",
                "Sign-in complete. You can close this tab and return to StealthStudy.",
                ok=True,
            )
        )
    ...
    async def cloud_auth_callback(code: str = "", state: str = "", error: str = ""):
        ...
        signin_failed_detail = (
            "Close this tab and try signing in again from StealthStudy."
        )
        if error:
            return HTMLResponse(
                _browser_page(
        ...
        return HTMLResponse(
            _browser_page(
                "Signed in",
                "You're signed in to StealthStudy Cloud. "
                "You can close this tab and return to StealthStudy.",
            )
        )
    ...
    async def ws_session(ws: WebSocket, session_id: str) -> None:
        ...
        if not _origin_allowed(ws.headers.get("origin")):
            await ws.close(code=1008)
            return
        await ws.accept(subprotocol="StealthStudy" if api_token else None)
