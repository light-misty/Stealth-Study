...
`/v1/cloud/status` inside `create_app()`, and `stealth_study/campus/config.py` — project-owned code, not
...
The 2026-09-16 stage verification widens it by one module again: `stealth_study/automation/store.py`
...
`stealth_study/server/run.py` initializes the unified log config at sidecar startup, and `app.py`
gains the request-context middleware plus the `POST /v1/logs/frontend` ingest endpoint
(startup timestamp file naming + 50MB/daily rotation live in new file `stealth_study/logging_setup.py`,
...
# this list — and outside `stealth_study/campus/`, which the feature owns — means the intrusion has
# spread and must go back through review.
BACKEND_PATCH = {
    "M\tstealth_study/campus/config.py",
    "M\tstealth_study/server/app.py",
    "M\tstealth_study/automation/store.py",
    # logging-system 分支：sidecar 启动时初始化统一日志
    "M\tstealth_study/server/run.py",
    # i18n-chinese-coverage 分支：错误响应附带稳定 error_code（文案仍为原英文，见 stealth_study/errors.py）
    "M\tstealth_study/server/manager.py",
    "M\tstealth_study/skills/store.py",
    # skip-question-card 分支（OPE-153）：ask 工具的哨兵值与跳过结算、引擎对
    # 全跳过卡片的 denied 判定、Slack 镜像为问题附带 Skip 按钮
    "M\tstealth_study/tools/ask.py",
    "M\tstealth_study/engine.py",
    "M\tstealth_study/interactions.py",
    # i18n-chinese-coverage 分支续：personas 域的清单/导出异常改抛带代号类型，云端画廊
    # 与 install/export 端点随之附带 error_code（裸文本照旧保留）
    "M\tstealth_study/personas/registry.py",
    # fix/study-agent-persona 分支：默认人设（cowork）系统提示词与元数据学习向改造
    "M\tstealth_study/agents/cowork.py",
    "M\tstealth_study/cloud.py",
    # i18n-chinese-coverage 分支续：项目命名异常改抛带代号类型（定时任务与绑定域）
    "M\tstealth_study/projects.py",
    # i18n-chinese-coverage 分支续：Slack 成员目录的未分类异常改用 forwarded_error 透传原文
    "M\tstealth_study/connectors/slack_directory.py",
    # i18n-chinese-coverage 分支续：连接器与 PDF 检查的自写守卫消息带代号
    "M\tstealth_study/connectors/setup.py",
    "M\tstealth_study/pdf_support.py",
    "M\tstealth_study/providers/registry.py",
}
CAMPUS_OWNED_PREFIX = "stealth_study/campus/"
# campus 挂载、日志系统、inbox 修复与错误代号四条分支各自的 app.py 增量预算
APP_PY_PATCHES = {
    "40\t2\tstealth_study/server/app.py",
    "34\t1\tstealth_study/server/app.py",
    "6\t3\tstealth_study/server/app.py",
    "27\t19\tstealth_study/server/app.py",
    "36\t25\tstealth_study/server/app.py",
    "44\t30\tstealth_study/server/app.py",
    # fix/rename-brand-leftovers：包重命名 + 认证头改写后的真实增量（需 -M 配对）。
    "4\t4\t{ss => stealth_study}/server/app.py",
}
...
def test_no_other_backend_module_changed_against_the_base_revision() -> None:
    ...
    status = _git("diff", "--name-status", base, "--", "stealth_study/")
...
def test_app_py_keeps_its_registered_budget_against_the_base_revision() -> None:
    ...
    if base is None or _on_the_base_revision(base):
        pytest.skip("no base revision to compare against from this checkout")
    numstat = _git("diff", "--numstat", "-M", base, "--", "ss/server/app.py", "stealth_study/server/app.py").strip()
...
def test_create_app_serves_the_campus_health_endpoint_behind_the_sidecar_token(
    ...
    assert client.get(HEALTH_PATH, headers={"x-stealthstudy-token": "wrong"}).status_code == 401
    authorized = client.get(HEALTH_PATH, headers={"x-stealthstudy-token": token})
...
def test_create_app_leaves_the_existing_endpoints_alone(
    ...
    monkeypatch.setenv("COWORKER_API_TOKEN", token)
    client = TestClient(create_app(SessionManager(data_dir=tmp_path / "data")))
    headers = {"x-stealthstudy-token": token}
...
def test_the_sidecar_entry_point_serves_the_campus_health_endpoint(tmp_path: Path) -> None:
    ...
    from stealth_study.server.run import build_app
