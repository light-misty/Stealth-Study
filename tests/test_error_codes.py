...
from ss.errors import CODES, UNCLASSIFIED, _ERRNO_CODES, _TEXT_CODES, _WINDOWS_CODES, error_code, error_payload
...
@pytest.mark.parametrize(
    "status,expected",
    [(401, "AUTH_FAILED"), (403, "AUTH_FAILED"), (429, "RATE_LIMITED"), (404, "HTTP_NOT_FOUND"), (500, "HTTP_SERVER_ERROR")],
)
...
def test_chinese_error_strings_hold_no_english_prose(code: str) -> None:
    ...
    assert re.search(r"[A-Za-z]{2,}", re.sub(r"\{\{[^}]*\}\}", "", text)) is None, (
        f"error.{code.lower()} 中文值残留英文单词：{text!r}"
    )


def test_classifiers_never_invent_a_code_outside_the_registry() -> None:
    produced = set(_ERRNO_CODES.values()) | set(_WINDOWS_CODES.values()) | {c for _, c in _TEXT_CODES}
    assert produced <= CODES


def test_registry_codes_are_all_reachable() -> None:
    # 表里列了却永远不会产出的代号就是死文案，逼着要么接上要么删掉。
    reachable = set(_ERRNO_CODES.values()) | set(_WINDOWS_CODES.values()) | {c for _, c in _TEXT_CODES}
    reachable |= {
        "AUTH_FAILED",
        "CONFLICT",
